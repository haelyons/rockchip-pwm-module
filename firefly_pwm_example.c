...
static int firefly_pwm_status_update(struct firefly_pwm_data *pdata)
{
	...
	printk(KERN_ALERT "firefly_pwm: collecting status update\n");
...
ssize_t firefly_pwm_parse_dt(struct firefly_pwm_data *firefly_pdata, struct platform_device *pdev)
{
	...
	printk(KERN_ALERT "firefly_pwm: Parsing device tree\n");
...
static int firefly_pwm_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ALERT "firefly_pwm: Entering probe function\n");
	...
	/*
	firefly_pdata->pwm = pwm_get(firefly_pdata->pwm_id, "firefly_pwm");
	...
	*/
	...
	// pwm_request is deprecated
	firefly_pdata->chip = pwm_get(&pdev->dev, "firefly_pwm"); 
    if (IS_ERR(firefly_pdata->chip)) { 
        dev_err(&pdev->dev, "unable to request PWM\n");
        ret = PTR_ERR(firefly_pdata->chip);
        goto err;
    }
	firefly_pdata->pwm = firefly_pdata->chip->pwms[firefly_pdata->pwm_id];
...
static int firefly_pwm_remove(struct platform_device *pdev)
{
	...
	pwm_disable(firefly_pdata->pwm); 
    pwm_put(firefly_pdata->chip);    
    return 0;
}
...
MODULE_ALIAS("platform:firefly_pwm");
MODULE_LICENSE("GPL");
